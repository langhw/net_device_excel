...
from netmiko.hp.hp_comware import HPComwareSSH, HPComwareBase
...
class BackupConfig(object):
    ...
    def printSum(self, msg):
        """打印结果汇总信息"""
        print('+' * 50)
        print("连接设备总数{}台，成功{}台, 失败{}台.".format(len(self.success + self.fail),
                                               len(self.success),
                                               len(self.fail)))
        print('所有设备已执行完成, 总耗时{:0.2f}s.'.format((msg).total_seconds()))
        print('+' * 50)
    ...
    def connectHandler(self, host, action=None):
        ...
        try:
            ...
            if host['protocol'].lower().strip() == 'ssh':
                ...
                if 'fortinet' in host['device_type']:
                    # 调用重写的MyFortinetSSH类
    ...
    def run_cmd(self, host, cmds, action=0):
        """执行命令和保存信息"""
        self.printPretty('设备_{}_开始执行......'.format(host['ip']))
    ...
    def run_t(self, host, action=1):
        ...
        if conn:
            try:
                output = "获取的设备提示符: {}".format(conn.find_prompt())
